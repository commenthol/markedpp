	...
	while (src) {
		...
		if ((cap = this.rules.ppnumberedheadings.exec(src))) {
			...
			this.tokens.push({
				...
				level: Lexer.range(opts.level, 1, 6, 3),
				minlevel: Lexer.range(opts.minlevel, 1, 6)
			});
			continue;
		}
...
Renderer.prototype.tableOfContents = function (toc, options) {
	...
	var	self = this,
		...
		level = options.level || 3,        // standard depth of TOC
 ...
 * @return {String} rendered output
 */
Renderer.prototype.numberedHeadings = function (maxLevel, minLevel) {
	var opts = this.composeOpts({
			level: maxLevel,
			minlevel: minLevel
		});
	if (this.options.tags) {
...
Renderer.prototype.composeOpts = function(obj) {
	var key,
		val,
		tmp = [];
	for (key in obj) {
 ...
 */
Parser.prototype.numberedHeadings = function (maxLevel, minLevel) {
	...
	return this.tokens.map(function(token){
		if (token.type === 'heading') {
			if (token.depth <= maxLevel &&
				token.depth >= minLevel &&
				token.text === token.raw)
			{
				var num = numbering.count(token.depth - minLevel + 1);
				token.number = num;
...
Parser.prototype.tok = function() {
	...
	switch (this.token.type) {
		...
		case 'ppnumberedheadings': {
			...
			this.tokens = this.numberedHeadings(this.token.level, this.token.minlevel);
			return this.renderer.numberedHeadings(this.token.level, this.token.minlevel);
		}
		case 'ppref': {
