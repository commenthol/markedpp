...
rules.pptoc = replace(rules.pptoc)
	...
	('_pptocCompat3_', rules._pptocCompat3_)
	();
rules.ppref = replace(rules.ppref)
	('_ppref_', rules._ppref_)
	('_pprefCompat1_', rules._pprefCompat1_)
	('_pprefCompat2_', rules._pprefCompat2_)
	();
 ...
 * Static Lexing
 */
InlineLexer.lex = function(src, links, options) {
		...
		if (options.numbered) {
			...
			if (self.options.numberedHeadings) {
				return (t.number ? t.number + ' ' : '') +
					'[' + self.sanitizeHeadings(t.raw) + '](#' +
					self.referenceId(t.text) + ')' + br;
			}
			else {
				return t.number +
					' [' + self.sanitizeHeadings(t.text) + '](#' +
					self.referenceId(t.text) + ')' + br;
			}
		}
		else {
