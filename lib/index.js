 ...
 * @credits Caolan McMahon <https://github.com/caolan/async>
 */ 
var async = {
...
function Lexer(options) {
	this.tokens = [];
	this.options = options || M.defaults;
	this.rules = rules;
}
...
Lexer.prototype.token = function(src) {
	...
	while (src) {
		...
		if ((cap = this.rules.ppnumberedheadings.exec(src))) {
			src = src.substring(cap[0].length);
			tmp = Lexer.splitCmds(cap[1] || cap[2]);
			this.tokens.push({
...
Lexer.lex = function(src, options, callback) {
	...
	Lexer.ppInclude(tokens, options, function(err, tokens){
		callback(err, tokens);
	});
};
 ...
 * @param {Object} options - options to overwrite
 * @prop {String} options.dirname - base directory from where to search files to include
 * @param {Function} callback - `function(err, tokens)`
 */
Lexer.ppInclude = function (tokens, options, callback) {
	...
	async.eachLimit(tokens, 5, function (token, done){
			...
			! _options.ppInclude[token.text] )
		{
			...
			fs.readFile(file, { encoding: 'utf8' }, function (err, src) {
				...
				Lexer.ppInclude( lexer.lex(src), _options, function (err, ntokens){
	...
	function (err) {
		...
		tokens.forEach(function(token, idx){
			...
			{
				_tokens.push({ type: 'ppinclude.start', text: token.text, indent: token.indent });
				lexed[token.text].forEach(function(token){
					_tokens.push(token);
				});
				_tokens.push({ type: 'ppinclude.end' });
			}
			else {
				_tokens.push(token);
			}
...
function markedpp(src, options, callback) {
	...
	options = merge({}, M.defaults, options || {});
